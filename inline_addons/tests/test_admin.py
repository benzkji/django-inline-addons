...
from inline_addons.models import Editor, FrontendUser
...
class AdminTestCase(TestCase):
    ...
    def changelist(self, user_version):
        url = reverse('admin:inline_addons_{}_changelist'.format(user_version))
    ...
    def change(self, user_version, id):
        url = reverse(
            'admin:inline_addons_{}_change'.format(user_version),
            args=[id]
        )
        response = self.client.get(url, follow=True)
        self.assertEqual(response.status_code, 200)
    ...
    def password_change(self, user_version, id):
        url = reverse(
            'admin:inline_addons_{}_password_change'.format(user_version),
            args=[id]
        )
        response = self.client.get(url, follow=True)
        self.assertEqual(response.status_code, 200)
